...
fn main() {
    ...
    let onmessage = Closure::<dyn FnMut(MessageEvent)>::new(move |msg: MessageEvent| {
        ...
        *GRAD.write().unwrap() = Some(get_grad());
        ...
        web_sys::console::log_1(&format!("Plotting initiated. Width={}, Height={}, cx={} cy={} hSpan={}", ctx.win_width, ctx.win_height, ctx.center_x, ctx.center_y, ctx.horizontal_span).into());
        ...
        let width = ctx.win_width;
        let height = ctx.win_height;
        *rgb_data_ref.borrow_mut() = Some(vec![0 as u8; 4 * width * height]);

        let _ = draw_plot_cpu(&mut ctx, context_ref.clone(), plot_ref.clone(), rgb_data_ref.clone());
    });
...
fn draw_plot_cpu(
...
) {
    {
        ...
        let (y0, symmetry) = ctx.revert_y(0.0);
        ...
        let y1 = y0.min(y_max);
        let y2 = y1 + 1;
        let y3 = if y1 > 0 { (y1 * 2 + 1 - symmetry_shift).min(y_max) } else { 0 };
        ...
        for (cur_y, row) in plot.iter().enumerate() {
            for (cur_x, val) in row.iter().enumerate() {
                ...
                let rel_val = (val.calculated_value() - ctx.min_value) / (ctx.max_value - ctx.min_value);
                let color = grad.at(rel_val).to_rgba8();
    ...
    if ctx.total_rows_processed < ctx.win_height {
        ...
        let cl = Closure::<dyn FnMut()>::new(move || {
            draw_plot_cpu(&mut ctx_cl, context_ref.clone(), plot_ref.clone(), data_ref.clone());
        });

        scope.request_animation_frame(cl.as_ref().unchecked_ref()).unwrap();
